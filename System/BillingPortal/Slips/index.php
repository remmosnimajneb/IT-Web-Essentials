...
/* Configue Filter Options */
	...
	/* 2. Client */
		...
		if($ClientName != "All"){
			$WHEREClause .= " AND `ClientName` = '" . EscapeSQLEntry($ClientName) . "'";
		}
		...
		<div class="content">
			...
			<!-- Previous and Next Day Toggles -->
				<div class="row">
					...
					<div class="column"><h2 style="text-align: center;">Slips for: <?php echo $Date; ?></h2></div>
			...
			<!-- Slips and Expenses -->
				...
				<h2>Time Slips</h2>
				<?php 
					$Query = "SELECT * FROM `Slips` WHERE `TSType` = 'TS' " . $WHEREClause . "";
				...
				<table class="alt"> 
				  <thead>
				    <tr>
				      <th scope="col" style="width:35px">TS ID</th>
				  ...
				  <tbody>
				  	<?php
						foreach ($records as $row) {
							echo "<tr>";
								echo "<td data-label='TS ID'>" . $row['TSID'] . "</td>";
								...
								echo "<td data-label='Edit'><a href='Slip.php?ID=" . $row['TSID'] . "'>Edit</a></td>";
							echo "</tr>";
						}
				...
				<h2>Expenses</h2>
				<?php 
					$Query = "SELECT * FROM `Slips` WHERE `TSType` = 'Expense' " . $WHEREClause . "";
			  		$stm = $DatabaseConnection->prepare($Query);
					$stm->execute();
					$records = $stm->fetchAll();
					$RowCount = $stm->rowCount();
