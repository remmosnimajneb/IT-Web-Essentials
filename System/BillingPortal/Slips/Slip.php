...
if(isset($_POST['Action'])){
	...
	if($_POST['Action'] == "Edit"){
		...
		if($_POST['DeleteSlip'] == "Yes"){
			$Query = "DELETE FROM `Slips` WHERE `TSID` = " . EscapeSQLEntry($_POST['ID']);
...
if(isset($_GET['ID']) && $_GET['ID'] != ""){
	$Query = "SELECT * FROM `Slips` WHERE `TSID` = " . EscapeSQLEntry($_GET['ID']);
	$stm = $DatabaseConnection->prepare($Query);
	$stm->execute();
	$Slip = $stm->fetchAll();
	$Slip = $Slip[0];
													...
													<?php 
														...
														foreach ($Clients as $Client) {
															echo "<option value=" . $Client['Slug'] . "";
																if($Slip['ClientName'] == $Client['Slug']){
																	echo " selected='selected'";
																}
