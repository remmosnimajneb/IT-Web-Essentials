...
if(isset($_GET['ID']) && $_GET['ID'] != ""){
	$Query = "SELECT * FROM `Invoice` AS I INNER JOIN `Clients` AS C ON C.`ClientID` = I.`ClientID` WHERE `InvoiceID` = " . EscapeSQLEntry($_GET['ID']);
	...
	<section id="content" class="default">
		...
		<div class="content">
			...
			<table class="alt">
				...
				<tbody id="SlipsBody">
					<?php
						$Query = "SELECT * FROM `Slips` WHERE `InvoiceID` = " . EscapeSQLEntry($_GET['ID']) . " ORDER BY `StartDate` ASC, `TSID` ASC";
						$stm = $DatabaseConnection->prepare($Query);
						$stm->execute();
						$Slips = $stm->fetchAll();
						foreach ($Slips as $Slip) {
					?>
						<tr id="Row_<?php echo $Slip['TSID']; ?>">
							<td><a href="../../Slips/Slip.php?ID=<?php echo $Slip['TSID']; ?>"><?php echo $Slip['TSID']; ?></a></td>
							...
							<td>
								<select onchange="Changed(<?php echo $Slip['TSID']; ?>)" autocomplete="off" id="SlipStatus_<?php echo $Slip['TSID']; ?>">
							...
							<td><input type="checkbox" name="Delete_<?php echo $Slip['TSID']; ?>" id="Delete_<?php echo $Slip['TSID']; ?>" onchange="Changed(<?php echo $Slip['TSID']; ?>)"> <label for="Delete_<?php echo $Slip['TSID']; ?>"></label>
							<td><button onclick="Save(<?php echo $Slip['TSID']; ?>)" disabled="disabled" id="Save_<?php echo $Slip['TSID']; ?>">Save</button></td>
						</tr>
					<?php
						}
					?>
